...
var currentStatus = $('div.status>div.value').text();//$('div.status').html();
...
function initFormElements() {
  FIELDS = {
    ...
    'TIME_TYPE': $('#time_entry_custom_field_values_6'),
    'ISSUE_START_DATE': $('#issue_start_date'),
    'ISSUE_DUE_DATE': $('#issue_due_date'),
    'ORDER': $('#issue_custom_field_values_7'),
    'CATEGORY_ID': $('#issue_category_id'),
    'TAG': $('#issue_custom_field_values_8'),
  ...
  if ((document.location.href.indexOf('time_entries') == - 1) || (document.location.href.indexOf('time_entries') > 0 && document.location.href.indexOf('edit') == - 1)
  ) {
    // set default values
...
function addMoreButton() {
  var buttonHtml = '<a href="#" class="btn btn-default dropdown-toggle" type="button" id="moreMenu" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">' +
  TEXT.MORE +
  ...
  if ($('a.icon-del')) {
    ...
    $('#moreMenuList').prepend($('<li>').append($('a.icon-del').removeClass('icon icon-del').prepend('<span class="glyphicon glyphicon-trash"></span> ')));
  }
  if ($('a.icon-copy')) {
    $('a.icon-copy').last().detach();
    $('a.icon-copy').first().removeClass('btn btn-default');
    $('#moreMenuList').prepend($('<li>').append($('a.icon-copy').first().removeClass('icon icon-copy').prepend('<span class="glyphicon glyphicon-duplicate"></span> ')));
  }
    ...
    $lis.slice(1, 8).each(function () {
      ...
      $li.children('ul').children('li').each(function () {
        ...
        if ($chidLi.children('a').hasClass('icon-checked')) {
          $chidLi.children('a').removeClass('icon-checked').prepend('<span class="glyphicon glyphicon-ok"></span> ');
        }
...
exportFunction(showConfigLightBoxOnComplete, unsafeWindow, {
  defineAs: 'showConfigLightBoxOnComplete'
});
var showConfigLightBoxOnClosed = function () {
  $('#config_form').off('.save_config');
  $('#configFormContainer').hide();
};
exportFunction(showConfigLightBoxOnClosed, unsafeWindow, {
...
exportFunction(showConfig, unsafeWindow, {
  defineAs: 'showConfig'
});
/**
...
function canStartProgress() {
  return (currentStatus == STATUS.NEW.TEXT || currentStatus == STATUS.FEEDBACK.TEXT || currentStatus == STATUS.REVIEW_FAILED.TEXT || currentStatus == STATUS.VERIFY_FAILED.TEXT || currentStatus == STATUS.FROZEN.TEXT);
} /**
 * Can start test
 *
 * @returns {boolean}
...
function formReturnToPreviousStateAfterPopupClose() {
  ...
  $('#update h3').show().parent().parent().css({
    'width': 'auto',
    'height': 'auto'
  });
...
exportFunction(resolveIssueOnComplete, unsafeWindow, {
  defineAs: 'resolveIssueOnComplete'
});
var resolveIssueOnClosed = function () {
  formReturnToPreviousStateAfterPopupClose();
  $('#issue-form').off('.resolve');
};
exportFunction(resolveIssueOnClosed, unsafeWindow, {
  defineAs: 'resolveIssueOnClosed'
});
...
exportFunction(_showReviewResultPopupOnComplete, unsafeWindow, {
  defineAs: '_showReviewResultPopupOnComplete'
});
...
exportFunction(_showReviewResultPopupOnClosed, unsafeWindow, {
  defineAs: '_showReviewResultPopupOnClosed'
});
...
exportFunction(_showLogTimePopupOnComplete, unsafeWindow, {
  defineAs: '_showLogTimePopupOnComplete'
});
var _showLogTimePopupOnClosed = function () {
  $('#new_time_entry').off('.log_time');
  $('#log_time_container').hide();
};
exportFunction(_showLogTimePopupOnClosed, unsafeWindow, {
...
exportFunction(startProgress, unsafeWindow, {
  defineAs: 'startProgress'
});
exportFunction(startTesting, unsafeWindow, {
  defineAs: 'startTesting'
});
exportFunction(testFailed, unsafeWindow, {
  defineAs: 'testFailed'
});
exportFunction(testPassed, unsafeWindow, {
  defineAs: 'testPassed'
});
...
exportFunction(resolveIssue, unsafeWindow, {
  defineAs: 'resolveIssue'
});
exportFunction(startReview, unsafeWindow, {
  defineAs: 'startReview'
});
exportFunction(reviewPassed, unsafeWindow, {
  defineAs: 'reviewPassed'
});
exportFunction(reviewFailed, unsafeWindow, {
  defineAs: 'reviewFailed'
});
exportFunction(_showReviewResultPopup, unsafeWindow, {
  defineAs: '_showReviewResultPopup'
});
exportFunction(startTimer, unsafeWindow, {
  defineAs: 'startTimer'
});
exportFunction(stopTimer, unsafeWindow, {
  defineAs: 'stopTimer'
});
exportFunction(isTimerStarted, unsafeWindow, {
  defineAs: 'isTimerStarted'
});
exportFunction(isTimerShown, unsafeWindow, {
  defineAs: 'isTimerShown'
});
exportFunction(getTimerTime, unsafeWindow, {
  defineAs: 'getTimerTime'
});
exportFunction(getTimeDiff, unsafeWindow, {
  defineAs: 'getTimeDiff'
});
exportFunction(logTimerTime, unsafeWindow, {
  defineAs: 'logTimerTime'
});
exportFunction(_showLogTimePopup, unsafeWindow, {
  defineAs: '_showLogTimePopup'
});
exportFunction(formPrepareToShowInPopup, unsafeWindow, {
  defineAs: 'formPrepareToShowInPopup'
});
exportFunction(formReturnToPreviousStateAfterPopupClose, unsafeWindow, {
  defineAs: 'formReturnToPreviousStateAfterPopupClose'
});
exportFunction(insertRtfBfqTable, unsafeWindow, {
  defineAs: 'insertRtfBfqTable'
});
